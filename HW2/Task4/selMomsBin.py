...
def calcSampleAverage():
    ...
    for j in range(5):
        for i in range(len(cnts)):
            data[j][i] = round(sum(binomData[i][j]) / len(binomData[i][j]), 3)
...
def calcSampleVar():
    ...
    for i in range(len(cnts)):
        arr1 = []
        for j in range(5):
            average = sum(binomData[i][j]) / len(binomData[i][j])
            arr = []

            for k in range(cnts[i]):
                arr.append((binomData[i][j][k] - average) ** 2)
...
def draw_table_inaccuracy_average():
    draw_table(calcInaccuracy(35.5, calcSampleAverage()),
               title=r'Inaccuracy of sample average $\overline{X}$')


def draw_table_inaccuracy_var():
    draw_table(calcInaccuracy(17.75, calcSampleVar()),
